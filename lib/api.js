...
api.get = function(resource, id, parameters) {
  return this.getToken().then(_.bind(function(token) {
    ...
    request(
      {url: this.urlFor(resource, id), qs: parameters, headers: {'Authorization': 'Bearer ' + token}, json: true},
      resolveResponse(deferred)
    );
...
api.index = function(resource, parameters) {
  return this.getToken().then(_.bind(function(token) {
    ...
    request({url: this.urlFor(resource), qs: parameters, headers: {'Authorization': 'Bearer ' + token}, json: true}, resolveResponse(deferred));
...
function resolveResponse(deferred) {
